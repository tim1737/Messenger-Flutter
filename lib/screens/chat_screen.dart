...
import 'package:chateau_chat_gen/helpers.dart';
...
import '../theme.dart';
import '../widgets/alert_error_chat.dart';
...
class _ChatScreenState extends State<ChatScreen> {
  late StreamSubscription<int> unreadCountSub;

  @override
  void initState() {
    super.initState();

    unreadCountSub = StreamChannel.of(context)
        .channel
        .state!
        .unreadCountStream
        .listen(_unreadCoundMess);
  }

  Future<void> _unreadCoundMess(int count) async {
    if (count > 0) {
          ...
          Expanded(
            child: MessageListCore(
              ...
              messageListBuilder: (context, messages) =>
                  _ChateListGen(messages: messages),
            ),
          ),
          const _ActionBar(),
        ],
...
class _ChateListGen extends StatelessWidget {
  const _ChateListGen({
    Key? key,
      ...
      child: ListView.separated(
        ...
        separatorBuilder: (context, index) {
          if (index == messages.length - 1) {
            return _DateLable(dateSendTime: messages[index].createdAt);
          }
          if (messages.length == 1) {
            ...
            if (!Jiffy(message.createdAt.toLocal())
              ...
              return _DateLable(
                dateSendTime: message.createdAt,
              );
            } else {
              return const SizedBox.shrink();
            }
...
class _DateLableState extends State<_DateLable> {
  ...
  void initState() {
    final createdAt = Jiffy(widget.dateSendTime);
    ...
    if (Jiffy(createdAt).isSame(now, Units.DAY)) {
      ...
      dayTime = createdAt.EEEE;
    } else if (Jiffy(createdAt).isAfter(
      Jiffy(now).subtract(years: 1),
      Units.DAY,
    )) {
      dayTime = createdAt.MMMd;
    } else {
      dayTime = createdAt.MMMd;
    }
...
class _GeneralTitle extends StatelessWidget {
  const _GeneralTitle({Key? key}) : super(key: key);
  @override
  Widget build(BuildContext context) {
